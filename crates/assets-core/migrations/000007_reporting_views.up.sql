...
CREATE VIEW account_monthly_sampled_balances AS
WITH relevant_accounts AS (
    -- Select all accounts you want to track
    SELECT id AS account_id, name AS account_name, account_type, account_subtype FROM accounts -- Removed account_code
),
month_series AS (
    -- Generate a series of the 1st day of each month
    SELECT date_trunc('month', dd)::date AS month_start_date
    FROM generate_series(
        date_trunc('month', (SELECT MIN(transaction_date) FROM transactions))::date, -- Start from the 1st of the month of the first transaction
        (SELECT MAX(transaction_date) FROM transactions)::date + INTERVAL '1 month', -- Go one month past last transaction date
        '1 month'::interval
    ) dd
),
accounts_months AS (
...
ORDER BY am.account_id, am.month_start_date;
...
SELECT
    ...
    a.account_type,
    a.account_subtype,
    a.symbol AS asset_symbol, -- This is from the accounts table
    a.quantity,
...
FROM
    accounts a
JOIN
    price_history ph ON a.symbol = ph.symbol -- Corrected: ph.symbol instead of ph.asset_symbol
WHERE
    a.symbol IS NOT NULL AND a.quantity IS NOT NULL AND a.quantity != 0
    AND a.account_subtype IN ('stocks', 'bonds', 'etf', 'mutual_fund', 'crypto', 'investment_account');

-- View for the latest market value of each asset-holding account
CREATE VIEW latest_account_market_values AS
WITH ranked_prices AS (
    SELECT
        ...
        ph.currency AS value_currency,
        (a.quantity * ph.price) AS market_value,
        ROW_NUMBER() OVER (PARTITION BY a.id ORDER BY ph.price_date DESC) as rn
    FROM
        accounts a
    JOIN
        price_history ph ON a.symbol = ph.symbol -- Corrected: ph.symbol instead of ph.asset_symbol
